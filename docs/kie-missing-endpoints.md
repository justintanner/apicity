...
| `POST /api/v1/aleph/generate`      | IMPLEMENTED | https://docs.kie.ai/runway-api/generate-aleph-video    | `prompt` (required), `videoUrl` (required), `callBackUrl`, `waterMark`, `uploadCn`, `aspectRatio`, `seed`, `referenceImage` | `kie.ts:715-725`; see render evidence below.      |
| `GET /api/v1/aleph/record-info`    | IMPLEMENTED | https://docs.kie.ai/runway-api/get-aleph-video-details | `taskId` (query, required)                                                                                                  | `kie.ts:727-733`; see render evidence below.      |

Aleph render evidence (2026-08-18): the implemented routes accepted and polled
four real submissions using a KIE-hosted `jump.mp4`, but every task finished
with `successFlag: 3`, `errorCode: 500`, and `internal error, please try again
later.` No successful Aleph render was observed. The failing task IDs were
`c0d088df35254cc3e7187fb7116b2efa`,
`aeceae14f47fb417e74fb27d521f2078`,
`4e3a051bf102e84e89dc7cf7ffba2bfe`, and
`b1afb1aee87195168ed9ac70a44b6541`.
...
| `POST /gemini-3-6-flash-openai/v1/chat/completions`                        | IMPLEMENTED | https://docs.kie.ai/market/gemini/gemini-3-6-flash-openai | `gemini-36-flash-openai.ts` gemini36FlashOpenai.v1.chat.completions |
...
| 4   | `POST /api/v1/aleph/generate` + `GET /api/v1/aleph/record-info`                    | —      | **Already gated** (generate)                                       | Implemented at `kie.ts:715-733`; `paid-endpoints.ts:174` already gates `api.v1.aleph.generate`.                                                                                                                                                                                                |
