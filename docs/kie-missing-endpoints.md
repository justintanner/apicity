...
the *same* endpoint (`POST /api/v1/jobs/createTask`) with a different `model`
value:
...
`@apicity/kie` implements many *create* routes without their paired *query*
route.
...
member for any of the 70. So callers who *do* validate explicitly against
...
This is a genuine surface gap but a different *kind* from the endpoint rows
