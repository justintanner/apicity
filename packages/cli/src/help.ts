...
const TOPIC_TEXT: Record<HelpTopic, () => string> = {
  ...
  environment: () =>
    [
      ...
      "",
      "A call fills the addressed provider's variables, and no other",
      "provider's; for each variable the first source that has one wins:",
      "",
      "  1. a value already set in the process environment",
      "  2. a literal in the env file: --env-file, else APICITY_ENV_FILE,",
      "     else ~/.config/apicity/.env",
      "  3. an op://<vault>/<item>/<field> reference, in the environment or",
      "     the env file (the `op run --env-file` format); all of a call's",
      "     references resolve in one `op inject`, with the configured token",
      "     or op's own sign-in",
      "  4. the vault convention op://<vault>/<VAR>/password, when --op-vault",
      "     and --op-token (APICITY_OP_VAULT, APICITY_OP_SERVICE_TOKEN) are",
      "     both set",
      "",
      "Persist the vault and token once, and every later call uses them:",
      "",
      "  apicity setup 1password --op-vault <vault> --op-token env:<VAR>",
      "",
