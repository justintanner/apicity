...
export function freeMediaUpload(opts?: FreeMediaUploadOptions): FreeMediaUploadProvider {
  ...
  async function makeFormRequest<T>(
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new FreeMediaUploadError(message, res.status, resBody);
      }

      return (await res.json()) as T;
    } catch (error) {
      clearTimeout(timeoutId);
      if (error instanceof FreeMediaUploadError) throw error;
      throw new FreeMediaUploadError(`Free request failed: ${error}`, 500);
    }
  }
