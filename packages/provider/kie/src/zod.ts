...
// ---------------------------------------------------------------------------
// WAN createTask models that match KieMediaWanModelAliasSchema. Each needs a
// per-model request member so CreateTaskRequestSchema /
// MediaGenerationRequestSchema accept it.
//
// The wan 2.2 / 2.5 ids stay alias-only — accepted by the alias, absent from
// KIE_MEDIA_MODELS. The five wan 2.6 ids are catalogued in KIE_MEDIA_MODELS,
// because guards and modelInputSchemas are total over KieMediaModel; the alias
// still covers future family members either way.
...
// ---------------------------------------------------------------------------
// Aleph (Runway video-to-video) — POST /api/v1/aleph/generate
// Docs: https://docs.kie.ai/runway-api/generate-aleph-video
// ---------------------------------------------------------------------------

// Aleph aspect ratios include 21:9 (wider than the base Runway generate enum).
export const AlephAspectRatioSchema = z.enum([
  "16:9",
  "9:16",
  "4:3",
  "3:4",
...
export const AlephGenerateRequestSchema = z.object({
  // Text prompt that guides the video-to-video transform.
  prompt: z.string().min(1),
  // Source video the Aleph model transforms according to the prompt.
  videoUrl: z.string().url(),
  callBackUrl: z.string().url().optional(),
  // Overlay text; empty string means no watermark.
  waterMark: z.string().optional(),
  // true → Alibaba Cloud OSS; false/omit → overseas R2.
  uploadCn: z.boolean().optional(),
  aspectRatio: AlephAspectRatioSchema.optional(),
  seed: z.number().int().optional(),
...
export type AlephGenerateRequest = z.input<typeof AlephGenerateRequestSchema>;
export type AlephGenerateRequestInput = AlephGenerateRequest;
export type AlephGenerateParsedRequest = z.output<
  typeof AlephGenerateRequestSchema
>;
export type AlephAspectRatio = z.infer<typeof AlephAspectRatioSchema>;
