...
## Real-world example: read prediction-market probabilities

Polymarket public-data endpoints don't require an API key. The factory
fronts three upstream hosts behind one provider:

| Namespace                | Host                       | Purpose                                |
| ------------------------ | -------------------------- | -------------------------------------- |
| `polymarket.get.gamma.*` | `gamma-api.polymarket.com` | events, markets, search, comments      |
| `polymarket.get.clob.*`  | `clob.polymarket.com`      | live order books, midpoint, last trade |
| `polymarket.get.data.*`  | `data-api.polymarket.com`  | positions, trades, holders, activity   |

End-to-end: look up a multi-market event by its slug, surface the
market-implied probabilities for each sub-market, then pull a live order
book for one of the CLOB token IDs that came back in the response. The
flow uses the same fixtures replayed by
[`tests/integration/polymarket-gamma-events.test.ts`](../../../tests/integration/polymarket-gamma-events.test.ts)
and
[`tests/integration/polymarket-clob-book.test.ts`](../../../tests/integration/polymarket-clob-book.test.ts).
...
// 1. Fetch a multi-market event by the slug that appears in its
//    polymarket.com URL. One Event groups related markets that resolve
//    as a unit (e.g. five "by date X" variants of the same question),
//    so `event.markets` is a list — not a single market.
const event = await polymarket.get.gamma.events.slug(
  "microstrategy-sell-any-bitcoin-in-2025",
);

console.log(event.title);
console.log(
  `  ${event.markets.length} markets · $${(event.volume ?? 0).toLocaleString()} cumulative volume`,
);

// 2. outcomes / outcomePrices / clobTokenIds come back as JSON-encoded
//    strings (a Gamma quirk — they're stored as text upstream). Parse
//    them before threading the CLOB token IDs through to other endpoints.
for (const m of event.markets) {
  const outcomes = JSON.parse(m.outcomes ?? "[]") as string[];
  const tokenIds = JSON.parse(m.clobTokenIds ?? "[]") as string[];
  const ask = m.bestAsk;
  const yesPercent =
    ask == null ? "—" : `${Math.round(Number(ask) * 100)}%`;

  console.log(`\n  ${m.question}`);
  console.log(
    `    ${outcomes.join(" / ")} → implied "${outcomes[0]}" probability: ${yesPercent}`,
  );
  console.log(`    YES token: ${tokenIds[0]?.slice(0, 16)}…`);
}

// 3. Pull the live order book for one of the YES-side CLOB tokens. The
//    CLOB returns bids in ascending price and asks in descending price,
//    so the top of book is the *last* element of each array.
const yesToken = (
  JSON.parse(event.markets[1].clobTokenIds ?? "[]") as string[]
)[0];

const book = await polymarket.get.clob.book({ token_id: yesToken });
const topBid = book.bids.at(-1);
const topAsk = book.asks.at(-1);

console.log(`\n  Live book — ${event.markets[1].question}`);
console.log(`    ${book.bids.length} bids · ${book.asks.length} asks`);
console.log(
  `    best bid ${topBid?.price ?? "—"}  ·  best ask ${topAsk?.price ?? "—"}`,
);
```

Sample output (real values from the recorded fixtures):

```
MicroStrategy sells any Bitcoin by ___ ?
  5 markets · $22,739,245.342 cumulative volume

  MicroStrategy sells any Bitcoin in 2025?
    Yes / No → implied "Yes" probability: 0%
    YES token: 9359294921279812…

  MicroStrategy sells any Bitcoin by December 31, 2026?
    Yes / No → implied "Yes" probability: 42%
    YES token: 1111281915815054…

  MicroStrategy sells any Bitcoin by March 31, 2026?
    Yes / No → implied "Yes" probability: 0%
    YES token: 1085479783279584…

  MicroStrategy sells any Bitcoin by June 30, 2026?
    Yes / No → implied "Yes" probability: 29%
    YES token: 1102518281615431…

  MicroStrategy sells any Bitcoin by May 31, 2026?
    Yes / No → implied "Yes" probability: 15%
    YES token: 2571400796029338…

  Live book — MicroStrategy sells any Bitcoin by December 31, 2026?
    14 bids · 30 asks
    best bid 0.03  ·  best ask 0.97
```
