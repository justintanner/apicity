...
// tests/integration/anthropic-messages-code-review.test.ts. The recording
// goes through https://api.kie.ai/claude — a Claude-compatible gateway —
// because the apicity vault's ANTHROPIC_API_KEY was rotated upstream and
// returns 401 against api.anthropic.com. The wire-format response is
// genuine Anthropic JSON, so the typed @apicity/anthropic call site below
// is exactly what you'd write against api.anthropic.com directly.
function renderAnthropicExample() {
  return [
    ...
    "without any tools, no JSON-mode, no fine-tune. Every token, id, and",
    "content block below is mined from",
    ...
    "The recording goes through `https://api.kie.ai/claude` (a",
    "Claude-compatible gateway — see the bottom note); the on-the-wire",
    "payload, response shape, and Claude itself are unchanged.",
    "",
    "````typescript",
    ...
    "",
    "console.log(text);",
    '// → "BUG: `Buffer.concat` expects `Buffer[]` but receives `Uint8Array[]`,',
    "//    causing a type error at runtime in environments where `Uint8Array`",
    '//    instances are not automatically treated as `Buffer`."',
    "",
    "console.log(",
    "  `model=${result.model} stop=${result.stop_reason} ` +",
    "    `in=${result.usage.input_tokens} out=${result.usage.output_tokens}`,",
    ");",
    '// → "model=claude-sonnet-4-6 stop=end_turn in=1310 out=315"',
    "````",
    "",
    "**Notes**",
    "",
    ...
    "- Switch backends without touching call sites by passing `baseURL`",
    "  (and an auth-translating `fetch` if the gateway uses a header other",
    "  than `x-api-key`) to `createAnthropic`. The recorded HAR above",
    "  drives `https://api.kie.ai/claude` — kie's Claude gateway uses",
    "  `Authorization: Bearer`, so the integration test wraps `fetch` to",
    "  rewrite the header on the way out. The exact wrapper lives in",
    "  [`tests/integration/anthropic-messages-code-review.test.ts`](../../../tests/integration/anthropic-messages-code-review.test.ts).",
    "",
  ].join("\n");
}
