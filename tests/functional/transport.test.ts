...
import { createTransport } from "../../packages/provider/kimicoding/src/transport";
...
function createTestTransport(fetchImpl: typeof fetch, timeoutMs = 1000) {
...
describe("shared provider transport", () => {
  it("returns JSON for successful requests", async () => {
    ...
    const transport = createTestTransport(fetchImpl);
    await expect(
  ...
  it("aborts requests on timeout", async () => {
    const transport = createTestTransport(pendingAbortFetch(), 1);
  ...
  it("aborts requests from external signals", async () => {
    const transport = createTestTransport(pendingAbortFetch());
