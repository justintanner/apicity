...
describe("fal serverless apps queue", () => {
  ...
  it("should expose apps.queue.flush with payloadSchema", () => {
    const provider = fal({ apiKey: "fal-test-key" });
    expect(typeof provider.ai.v1.serverless.apps.queue.flush).toBe("function");
    expect(
      provider.ai.v1.serverless.apps.queue.flush.payloadSchema
    ).toBeDefined();
    expect(
      provider.ai.v1.serverless.apps.queue.flush.payloadSchema.method
    ).toBe("DELETE");
    expect(
      provider.ai.v1.serverless.apps.queue.flush.payloadSchema.fields.owner
    ).toBeDefined();
    expect(
      provider.ai.v1.serverless.apps.queue.flush.payloadSchema.fields.owner
        .required
    ...
    expect(
      provider.ai.v1.serverless.apps.queue.flush.payloadSchema.fields.name
    ).toBeDefined();
    expect(
      provider.ai.v1.serverless.apps.queue.flush.payloadSchema.fields.name
        .required
  ...
  it("should validate flush params — valid", () => {
    const provider = fal({ apiKey: "fal-test-key" });
    const result = provider.ai.v1.serverless.apps.queue.flush.validatePayload({
      owner: "user_123",
      name: "my-app",
    });
