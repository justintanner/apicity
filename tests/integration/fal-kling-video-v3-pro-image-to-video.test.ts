...
import { FalKlingVideoV3ProImageToVideoRequestSchema } from "@apicity/fal/zod";

describe("fal kling-video v3 pro image-to-video integration", () => {
  let ctx: PollyContext;

  beforeEach(() => {
    ctx = setupPollyIgnoringBody("fal/kling-video-v3-pro-image-to-video");
  });
  ...
  it("should generate a video from an image", async () => {
    ...
    const result = await provider.run.klingVideo.v3.pro.imageToVideo({
  ...
  it("should validate a valid payload", () => {
    ...
    const v = provider.run.klingVideo.v3.pro.imageToVideo.schema.safeParse({
      start_image_url: "https://example.com/img.png",
    });
    expect(v.success).toBe(true);
  });

  it("should reject payload missing required start_image_url", () => {
    const provider = createFal({ apiKey: "fal-test-key" });
    const v = provider.run.klingVideo.v3.pro.imageToVideo.schema.safeParse({});
  ...
  it("should expose the same function via run and post.run", () => {
    const provider = createFal({ apiKey: "fal-test-key" });
    expect(provider.run.klingVideo.v3.pro.imageToVideo).toBe(
      provider.post.run.klingVideo.v3.pro.imageToVideo
    );
  });
});
