...
import { FalLtx2p5ImageToVideoProRequestSchema } from "@apicity/fal/zod";

describe("fal lightricks ltx-2.5 image-to-video pro integration", () => {
  let ctx: PollyContext;

  beforeEach(() => {
    ctx = setupPollyIgnoringBody("fal/lightricks-ltx2p5-image-to-video-pro");
  });
  ...
  it("should generate a video from a start image", async () => {
    ...
    // Cheapest shape upstream offers: the shortest duration, the lower
    // resolution tier, the lowest frame rate, and no audio track.
    const result = await provider.run.lightricks.ltx2p5.imageToVideo.pro({
  ...
  it("should validate a minimal payload", () => {
    ...
    const v = provider.run.lightricks.ltx2p5.imageToVideo.pro.schema.safeParse({
  ...
  it("should reject an empty prompt and one over 5000 characters", () => {
    const provider = createFal({ apiKey: "fal-test-key" });
    const schema = provider.run.lightricks.ltx2p5.imageToVideo.pro.schema;
    expect(
      schema.safeParse({
        image_url: "https://example.com/img.png",
        prompt: "",
  ...
  it("should accept every documented duration and reject others", () => {
    ...
    for (const duration of [6, 8, 10, "auto"]) {
      expect(
        schema.safeParse({ ...base, duration }).success,
        `${duration}`
      ).toBe(true);
    }
    // 5 and 7 are neighbours upstream does not offer; the enum is closed.
  ...
  it("should bound resolution, aspect ratio, fps and camera motion", () => {
    ...
    expect(schema.safeParse({ ...base, resolution: "720p" }).success).toBe(
      true
    );
  ...
  it("should expose schema", () => {
    ...
    expect(schema).toBe(FalLtx2p5ImageToVideoProRequestSchema);
    expect(typeof schema.safeParse).toBe("function");
  });

  it("should expose the same function via run and post.run", () => {
    const provider = createFal({ apiKey: "fal-test-key" });
    expect(provider.run.lightricks.ltx2p5.imageToVideo.pro).toBe(
      provider.post.run.lightricks.ltx2p5.imageToVideo.pro
    );
  });
});
