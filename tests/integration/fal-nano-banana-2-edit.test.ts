...
import { FalNanoBanana2EditRequestSchema } from "@apicity/fal/zod";

describe("fal nano-banana-2 edit integration", () => {
  let ctx: PollyContext;

  beforeEach(() => {
    ctx = setupPollyIgnoringBody("fal/nano-banana-2-edit");
  });
  ...
  it("should edit an image with a prompt", async () => {
    ...
    const result = await provider.run.nanoBanana2.edit({
      prompt:
  ...
  it("should expose schema", () => {
    const provider = createFal({ apiKey: "fal-test-key" });
    const schema = provider.run.nanoBanana2.edit.schema;
    // Bind the identity, not just presence: the MCP server derives this
    // endpoint's tool input JSON Schema from `.schema`, so attaching a
    // sibling's schema here would ship a wrong tool contract silently.
    expect(schema).toBe(FalNanoBanana2EditRequestSchema);
    expect(typeof schema.safeParse).toBe("function");
  });

  it("should expose the same function via run and post.run", () => {
    const provider = createFal({ apiKey: "fal-test-key" });
    expect(provider.run.nanoBanana2.edit).toBe(
      provider.post.run.nanoBanana2.edit
    );
  });
});
