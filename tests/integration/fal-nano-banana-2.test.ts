...
describe("fal nano-banana-2 text-to-image integration", () => {
  let ctx: PollyContext;

  beforeEach(() => {
    ctx = setupPollyIgnoringBody("fal/nano-banana-2");
  });
  ...
  it("should generate an image from a text prompt", async () => {
    ...
    const result = await provider.run.nanoBanana2.textToImage({
      prompt:
