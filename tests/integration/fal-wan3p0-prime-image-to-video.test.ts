...
describe("fal alibaba wan3p0-prime image-to-video integration", () => {
  let ctx: PollyContext;

  beforeEach(() => {
    ctx = setupPolly("fal/wan3p0-prime-image-to-video");
  });
  ...
  it("should animate cat1.jpg into a short 480p video", async () => {
    ...
    const result = await provider.run.alibaba.wan3p0Prime.imageToVideo({
      prompt:
  ...
  it("should validate a minimal payload", () => {
    ...
    const result =
      provider.run.alibaba.wan3p0Prime.imageToVideo.schema.safeParse({
  ...
  it("should bound duration to the published 2-30 second range", () => {
    const provider = createFal({ apiKey: "fal-test-key" });
    const schema = provider.run.alibaba.wan3p0Prime.imageToVideo.schema;
  ...
  it("should expose the same function via run and post.run", () => {
    const provider = createFal({ apiKey: "fal-test-key" });
    expect(provider.run.alibaba.wan3p0Prime.imageToVideo).toBe(
      provider.post.run.alibaba.wan3p0Prime.imageToVideo
    );
  });
});
