...
describe("fal alibaba wan3p0 reference-to-video integration", () => {
  let ctx: PollyContext;

  beforeEach(() => {
    ctx = setupPolly("fal/wan3p0-reference-to-video");
  });
  ...
  it("should generate a short video from an image reference", async () => {
    ...
    const result = await provider.run.alibaba.wan3p0.referenceToVideo({
      prompt:
  ...
  it("should validate an image-reference payload", () => {
    ...
    const result =
      provider.run.alibaba.wan3p0.referenceToVideo.schema.safeParse({
  ...
  it("should enforce the published reference-media limits", () => {
    const provider = createFal({ apiKey: "fal-test-key" });
    const schema = provider.run.alibaba.wan3p0.referenceToVideo.schema;
  ...
  it("should expose the same function via run and post.run", () => {
    const provider = createFal({ apiKey: "fal-test-key" });
    expect(provider.run.alibaba.wan3p0.referenceToVideo).toBe(
      provider.post.run.alibaba.wan3p0.referenceToVideo
    );
  });
});
