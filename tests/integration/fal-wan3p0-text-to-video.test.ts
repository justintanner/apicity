...
describe("fal alibaba wan3p0 text-to-video integration", () => {
  let ctx: PollyContext;

  beforeEach(() => {
    ctx = setupPolly("fal/wan3p0-text-to-video");
  });
  ...
  it("should generate a short 480p video from a text prompt", async () => {
    ...
    const result = await provider.run.alibaba.wan3p0.textToVideo({
      prompt:
  ...
  it("should validate a minimal payload", () => {
    ...
    const result = provider.run.alibaba.wan3p0.textToVideo.schema.safeParse({
  ...
  it("should bound duration to the published 2-30 second range", () => {
    const provider = createFal({ apiKey: "fal-test-key" });
    const schema = provider.run.alibaba.wan3p0.textToVideo.schema;
    expect(
  ...
  it("should expose the same function via run and post.run", () => {
    const provider = createFal({ apiKey: "fal-test-key" });
    expect(provider.run.alibaba.wan3p0.textToVideo).toBe(
      provider.post.run.alibaba.wan3p0.textToVideo
    );
  });
});
