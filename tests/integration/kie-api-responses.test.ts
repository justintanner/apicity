...
describe("kie api v1 responses (unified GPT Codex)", () => {
  it("maps multimodal web-search requests to POST /api/v1/responses", async () => {
    ...
    const result = await provider.post.api.v1.responses({
      model: "gpt-5.1-codex",
      stream: false,
      input: [
        {
    ...
    expect(String(captured.input)).toBe("https://api.kie.ai/api/v1/responses");
  ...
  it("maps function tools with tool_choice auto", async () => {
    ...
    await provider.post.api.v1.responses({
      model: "gpt-5-codex",
      input: "Use the tool.",
      tools: [
        {
  ...
  it("rejects mixed web_search and function tools in schema metadata", () => {
    ...
    const parsed = provider.post.api.v1.responses.schema.safeParse({
      model: "gpt-5.2-codex",
  ...
  it("parses streaming responses events", async () => {
    ...
    const stream = await provider.post.api.v1.responses({
      model: "gpt-5.3-codex",
  ...
  it("surfaces HTTP 200 Kie auth envelopes as KieError 401", async () => {
    const provider = createKie({
      apiKey: "sk-test-key",
      fetch: async () =>
        jsonResponse({
