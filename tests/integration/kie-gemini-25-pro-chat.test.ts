...
import {
  ...
  type KieGemini25ProChatCompletionChunk,
  type KieGemini25ProChatCompletionsResult,
} from "@apicity/kie";
...
function assertAsyncIterable(
  value: KieGemini25ProChatCompletionsResult
): asserts value is AsyncIterable<KieGemini25ProChatCompletionChunk> {
...
describe("kie gemini 2.5 flash openai chat completions", () => {
  it("validates multimodal googleSearch and custom tool request payloads", () => {
    const valid = KieGemini25ProChatCompletionsRequestSchema.safeParse({
      messages: [
        {
          role: "user",
          content: [
    ...
    const customTool = KieGemini25ProChatCompletionsRequestSchema.safeParse({
      messages: [
    ...
    expect(
      KieGemini25ProChatCompletionsRequestSchema.safeParse({
        messages: [],
      }).success
    ).toBe(false);
    expect(
  ...
  it("posts requests with bearer auth and preserves usage details", async () => {
    ...
    const provider = createKie({
      apiKey: "kie-gemini-25-pro-test-key",
    ...
    const result = await provider.gemini25Pro.post.v1.chat.completions({
      messages: [
        {
          role: "developer",
    ...
    expect(headers.get("Authorization")).toBe(
      "Bearer kie-gemini-25-pro-test-key"
    );
  ...
  it("returns async chunks for event-stream responses", async () => {
    ...
    const chunks: KieGemini25ProChatCompletionChunk[] = [];
