...
import {
  ...
  type KieGemini3FlashChatCompletionChunk,
  type KieGemini3FlashChatCompletionsResult,
} from "@apicity/kie";
...
function assertAsyncIterable(
  value: KieGemini3FlashChatCompletionsResult
): asserts value is AsyncIterable<KieGemini3FlashChatCompletionChunk> {
  expect(typeof value).toBe("object");
  expect(value).not.toBeNull();
  expect(Symbol.asyncIterator in value).toBe(true);
}

describe("kie gemini 3 flash openai chat completions", () => {
  it("validates multimodal googleSearch and custom tool request payloads", () => {
    const valid = KieGemini3FlashChatCompletionsRequestSchema.safeParse({
      messages: [
        {
          role: "user",
          content: [
    ...
    const customTool = KieGemini3FlashChatCompletionsRequestSchema.safeParse({
      messages: [
    ...
    expect(
      KieGemini3FlashChatCompletionsRequestSchema.safeParse({
        messages: [],
      }).success
    ).toBe(false);
    expect(
  ...
  it("posts requests with bearer auth and preserves usage details", async () => {
    ...
    const provider = createKie({
      apiKey: "kie-gemini-3-flash-test-key",
    ...
    const result = await provider.gemini3Flash.post.v1.chat.completions({
      messages: [
        {
          role: "developer",
    ...
    expect(headers.get("Authorization")).toBe(
      "Bearer kie-gemini-3-flash-test-key"
    );
  ...
  it("returns async chunks for event-stream responses", async () => {
    ...
    const chunks: KieGemini3FlashChatCompletionChunk[] = [];
  ...
  it("surfaces openai-style error envelopes with status and type", async () => {
    ...
    await expect(
      provider.gemini3Flash.post.v1.chat.completions({
        messages: [
