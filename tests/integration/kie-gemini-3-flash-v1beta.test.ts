...
import {
  ...
  type KieGemini3FlashV1betamodelsStreamGenerateContentChunk,
  type KieGemini3FlashV1betamodelsStreamGenerateContentResult,
} from "@apicity/kie";
...
function assertAsyncIterable(
  value: KieGemini3FlashV1betamodelsStreamGenerateContentResult
): asserts value is AsyncIterable<KieGemini3FlashV1betamodelsStreamGenerateContentChunk> {
  expect(typeof value).toBe("object");
  expect(value).not.toBeNull();
  expect(Symbol.asyncIterator in value).toBe(true);
}

describe("kie gemini 3 flash v1beta", () => {
  it("validates multimodal tools and thinking request payloads", () => {
    const valid =
      KieGemini3FlashV1betamodelsStreamGenerateContentRequestSchema.safeParse({
        stream: true,
        contents: [
          {
            role: "user",
  ...
  it("posts non-streaming requests with X-Goog-Api-Key auth", async () => {
    ...
    const result =
      await provider.gemini.post.v1.models.gemini3FlashV1betamodels.streamGenerateContent(
        {
          stream: false,
          contents: [
            {
  ...
  it("returns async chunks for event-stream responses", async () => {
      ...
      fetch: async () =>
        new Response(
          [
            ...
            "",
            'data: {"usageMetadata":{"totalTokenCount":12},"modelVersion":"gemini-3-flash"}',
            "",
            "data: [DONE]",
            "",
          ].join("\n"),
    ...
    const chunks: KieGemini3FlashV1betamodelsStreamGenerateContentChunk[] = [];
