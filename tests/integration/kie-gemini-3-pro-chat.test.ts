...
import {
  ...
  type KieGemini3ProChatCompletionChunk,
  type KieGemini3ProChatCompletionsResult,
} from "@apicity/kie";
...
function assertAsyncIterable(
  value: KieGemini3ProChatCompletionsResult
): asserts value is AsyncIterable<KieGemini3ProChatCompletionChunk> {
  expect(typeof value).toBe("object");
  expect(value).not.toBeNull();
  expect(Symbol.asyncIterator in value).toBe(true);
}

describe("kie gemini 3 pro openai chat completions", () => {
  it("validates multimodal googleSearch and custom tool request payloads", () => {
    const valid = KieGemini3ProChatCompletionsRequestSchema.safeParse({
      messages: [
        {
          role: "user",
          content: [
    ...
    const customTool = KieGemini3ProChatCompletionsRequestSchema.safeParse({
      messages: [
    ...
    expect(
      KieGemini3ProChatCompletionsRequestSchema.safeParse({
        messages: [],
      }).success
    ).toBe(false);
    expect(
  ...
  it("posts requests with bearer auth and preserves usage details", async () => {
    ...
    const provider = createKie({
      apiKey: "kie-gemini-3-pro-test-key",
    ...
    const result = await provider.gemini3Pro.post.v1.chat.completions({
      messages: [
        {
          role: "developer",
    ...
    expect(headers.get("Authorization")).toBe(
      "Bearer kie-gemini-3-pro-test-key"
    );
  ...
  it("returns async chunks for event-stream responses", async () => {
    ...
    const chunks: KieGemini3ProChatCompletionChunk[] = [];
