...
import {
  ...
  type KieGemini35FlashOpenaiChatCompletionChunk,
  type KieGemini35FlashOpenaiChatCompletionsResult,
} from "@apicity/kie";
...
function assertAsyncIterable(
  value: KieGemini35FlashOpenaiChatCompletionsResult
): asserts value is AsyncIterable<KieGemini35FlashOpenaiChatCompletionChunk> {
  expect(typeof value).toBe("object");
  expect(value).not.toBeNull();
  expect(Symbol.asyncIterator in value).toBe(true);
}

describe("kie gemini 3.5 flash openai chat completions", () => {
  it("validates multimodal googleSearch and custom tool request payloads", () => {
    const valid = KieGemini35FlashOpenaiChatCompletionsRequestSchema.safeParse({
      messages: [
        {
          role: "user",
          content: [
    ...
    const customTool =
      KieGemini35FlashOpenaiChatCompletionsRequestSchema.safeParse({
        messages: [
  ...
  it("posts requests with bearer auth and preserves usage details", async () => {
    ...
    const provider = createKie({
      apiKey: "kie-gemini-3-5-flash-openai-test-key",
    ...
    const result = await provider.gemini35FlashOpenai.post.v1.chat.completions({
      messages: [
        {
          role: "developer",
    ...
    expect(headers.get("Authorization")).toBe(
      "Bearer kie-gemini-3-5-flash-openai-test-key"
    );
    ...
    expect(result.model).toBe("gemini-3-5-flash-openai");
  ...
  it("returns async chunks for event-stream responses", async () => {
    ...
    const chunks: KieGemini35FlashOpenaiChatCompletionChunk[] = [];
  ...
  it("surfaces openai-style error envelopes with status and type", async () => {
    ...
    await expect(
      provider.gemini35FlashOpenai.post.v1.chat.completions({
        messages: [
