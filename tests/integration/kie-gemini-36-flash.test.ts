...
import {
  ...
  type KieGemini36FlashStreamGenerateContentChunk,
  type KieGemini36FlashStreamGenerateContentResult,
} from "@apicity/kie";
...
function assertAsyncIterable(
  value: KieGemini36FlashStreamGenerateContentResult
): asserts value is AsyncIterable<KieGemini36FlashStreamGenerateContentChunk> {
  expect(typeof value).toBe("object");
  expect(value).not.toBeNull();
  expect(Symbol.asyncIterator in value).toBe(true);
}

describe("kie gemini 3.6 flash", () => {
  it("validates multimodal tools and thinking request payloads", () => {
    const valid = KieGemini36FlashStreamGenerateContentRequestSchema.safeParse({
      stream: true,
      contents: [
        {
          role: "user",
    ...
    expect(
      KieGemini36FlashStreamGenerateContentRequestSchema.safeParse({
        contents: [],
      }).success
    ).toBe(false);
    expect(
  ...
  it("posts non-streaming requests with X-Goog-Api-Key auth", async () => {
    ...
    const result =
      await provider.gemini.post.v1.models.gemini36Flash.streamGenerateContent({
        stream: false,
        contents: [
          {
            role: "user",
    ...
    expect(capturedUrl).toBe(
      "https://api.kie.ai/gemini/v1/models/gemini-3-6-flash:streamGenerateContent"
    );
  ...
  it("returns async chunks for event-stream responses", async () => {
      ...
      fetch: async () =>
        new Response(
          [
            ...
            "",
            'data: {"usageMetadata":{"totalTokenCount":12},"modelVersion":"gemini-3-6-flash"}',
            "",
            "data: [DONE]",
            "",
          ].join("\n"),
    ...
    const chunks: KieGemini36FlashStreamGenerateContentChunk[] = [];
