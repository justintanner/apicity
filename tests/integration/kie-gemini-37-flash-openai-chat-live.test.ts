...
/**
 * Live Polly recording for Gemini 3.7 Flash OpenAI chat completions.
 * Prefer free auth-error path (invalid key) so recording does not spend credits.
 */
describe("kie gemini 3.7 flash openai chat live", () => {
  ...
  it("rejects invalid API key with KieError", async () => {
    ctx = setupPolly("kie/gemini-37-flash-openai-chat-auth-error");
    const provider = createKie({
      // Intentionally invalid: free 401 path for the HAR fixture.
      apiKey: "sk-invalid-kie-gemini-37-flash-openai",
    });
