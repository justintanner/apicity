...
import {
  ...
  type KieGemini38FlashStreamGenerateContentChunk,
  type KieGemini38FlashStreamGenerateContentResult,
} from "@apicity/kie";
...
function assertAsyncIterable(
  value: KieGemini38FlashStreamGenerateContentResult
): asserts value is AsyncIterable<KieGemini38FlashStreamGenerateContentChunk> {
  expect(typeof value).toBe("object");
  expect(value).not.toBeNull();
  expect(Symbol.asyncIterator in value).toBe(true);
}

describe("kie gemini 3.8 flash", () => {
  it("validates multimodal tools and thinking request payloads", () => {
    const valid = KieGemini38FlashStreamGenerateContentRequestSchema.safeParse({
      stream: true,
      contents: [
        {
          role: "user",
    ...
    expect(
      KieGemini38FlashStreamGenerateContentRequestSchema.safeParse({
        contents: [],
      }).success
    ).toBe(false);
    expect(
  ...
  it("posts non-streaming requests with X-Goog-Api-Key auth", async () => {
    ...
    const result =
      await provider.gemini.post.v1.models.gemini38Flash.streamGenerateContent({
        stream: false,
        contents: [
          {
            role: "user",
  ...
  it("returns async chunks for event-stream responses", async () => {
      ...
      fetch: async () =>
        new Response(
          [
            ...
            "",
            'data: {"usageMetadata":{"totalTokenCount":12},"modelVersion":"gemini-3-8-flash"}',
            "",
            "data: [DONE]",
            "",
          ].join("\n"),
    ...
    const chunks: KieGemini38FlashStreamGenerateContentChunk[] = [];
