...
describe("kie gpt-image-2-5-flare-image-to-image integration", () => {
  ...
  it(
    ...
    { timeout: 600_000 },
    async () => {
      ctx = setupPolly("kie/gpt-image-2-5-flare-image-to-image");
      ...
      // Same-variant secondary reference for the REQ-009 decision rule
      // (plans/ac-1m1js4 A-4): same ratio and explicit 1K as the control.
      const request = {
        model: "gpt-image-2-5-flare-image-to-image",
        input: {
          prompt:
            "Restyle this product image as a premium studio advertisement with soft rim lighting and a matte charcoal backdrop.",
          input_urls: [
  ...
  it("should validate payload via schema", () => {
    ...
    const family = GptImage25FlareImageToImageRequestSchema.safeParse(oneKOnly);
