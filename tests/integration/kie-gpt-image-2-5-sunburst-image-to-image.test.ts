...
describe("kie gpt-image-2-5-sunburst-image-to-image integration", () => {
  ...
  it(
    ...
    { timeout: 600_000 },
    async () => {
      ctx = setupPolly("kie/gpt-image-2-5-sunburst-image-to-image");
      ...
      // Secondary reference for the REQ-009 decision rule (plans/ac-1m1js4
      // A-4): same input image, ratio and explicit 1K as the flare pair.
      const request = {
        model: "gpt-image-2-5-sunburst-image-to-image",
        input: {
          prompt:
            "Place this product on a sunlit wooden table beside a window with sheer curtains, editorial catalogue style.",
          input_urls: [
  ...
  it("should validate payload via schema", () => {
    ...
    const family =
      GptImage25SunburstImageToImageRequestSchema.safeParse(oneKOnly);
