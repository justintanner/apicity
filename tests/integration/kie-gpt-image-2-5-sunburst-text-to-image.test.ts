...
describe("kie gpt-image-2-5-sunburst-text-to-image integration", () => {
  ...
  it(
    ...
    { timeout: 600_000 },
    async () => {
      ctx = setupPolly("kie/gpt-image-2-5-sunburst-text-to-image");
      ...
      // The same-ratio explicit-1K control the REQ-009 decision rule
      // compares the flare-text-to-image probe against (plans/ac-1m1js4).
      const request = {
        model: "gpt-image-2-5-sunburst-text-to-image",
        input: {
          prompt:
            "A hot-air balloon festival over a river valley at sunrise, dozens of balloons, soft morning haze, photorealistic.",
          aspect_ratio: "16:9",
          resolution: "1K",
        },
  ...
  it("should validate payload via schema", () => {
    ...
    const family =
      GptImage25SunburstTextToImageRequestSchema.safeParse(oneKOnly);
