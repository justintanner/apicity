...
const KLING_V3_TURBO_TEXT_TO_VIDEO_1080P_REQUEST = {
...
describe("kie kling/v3-turbo-text-to-video 1080p/10s integration", () => {
  ...
  it(
    "creates a 1080p/10s task and polls to a successful media result",
    { timeout: 600_000 },
    async () => {
      ctx = setupPolly("kie/kling-v3-turbo-text-to-video-1080p");
      ...
      const task = await provider.post.api.v1.jobs.createTask(
        KLING_V3_TURBO_TEXT_TO_VIDEO_1080P_REQUEST,
        mintKieCreateTaskOtp(KLING_V3_TURBO_TEXT_TO_VIDEO_1080P_REQUEST)
      );
