...
describe("kie gpt 5.5 responses", () => {
  it("maps multimodal web-search requests to POST /codex/v1/responses", async () => {
    ...
    const result = await provider.post.codex.v1.responses({
      model: "gpt-5-5",
      stream: false,
      input: [
        {
  ...
  it("maps function tools with tool_choice auto", async () => {
    ...
    await provider.post.codex.v1.responses({
      model: "gpt-5-5",
      input: "Use the tool.",
      tools: [
        {
  ...
  it("rejects mixed web_search and function tools in schema metadata", () => {
    ...
    const parsed = provider.post.codex.v1.responses.schema.safeParse({
      model: "gpt-5-5",
  ...
  it("parses streaming responses events", async () => {
    ...
    const stream = await provider.post.codex.v1.responses({
      model: "gpt-5-5",
  ...
  it("surfaces upstream error payloads with KieError code", async () => {
    ...
    await expect(
      provider.post.codex.v1.responses({
        model: "gpt-5-5",
        input: "Hello.",
      })
