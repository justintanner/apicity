...
describe("kie seedream/5-pro-text-to-image integration", () => {
  ...
  it(
    ...
    { timeout: 600_000 },
    async () => {
      ctx = setupPolly("kie/seedream-5-pro-text-to-image");
      ...
      const request = {
        ...
        input: {
          prompt:
            "A full-process cafe design tool for entrepreneurs and designers, covering store layout, functional zoning, decoration style, and equipment selection. A clean promotional image for a cafe design SaaS product.",
          aspect_ratio: "16:9",
          quality: "basic",
          output_format: "png",
          nsfw_checker: false,
  ...
  it("should validate payload via schema", () => {
    ...
    const ok = provider.post.api.v1.jobs.createTask.schema.safeParse({
      model: "seedream/5-pro-text-to-image",
      input: {
        prompt: "A promotional image for a cafe design SaaS product",
        aspect_ratio: "1:1",
        quality: "high",
        output_format: "jpeg",
      },
