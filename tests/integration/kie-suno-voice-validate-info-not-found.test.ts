...
const RECORDING_NAME = "kie/suno/voice-validate-info-not-found";
const MISSING_TASK_ID = "apicity-test-nonexistent-task-id-do-not-record-real";

describe("kie suno voice validate-info (not found)", () => {
  ...
  it("returns the null-data success envelope for a missing task", async () => {
    ...
    const validateInfo = provider.suno.get.api.v1.voice.validateInfo;

    const result = await validateInfo(MISSING_TASK_ID);

    expect(result).toEqual({ code: 200, msg: "success", data: null });
    expect(validateInfo.responseSchema.safeParse(result).success).toBe(true);
  });
});
