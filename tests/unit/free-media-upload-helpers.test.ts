// Tests for @apicity/free-media-upload helper functions — no API calls, mocks the provider
import { describe, it, expect, vi, type Mock } from "vitest";
import { uploadToAnyHost } from "../../packages/provider/free-media-upload/src/uploadToAnyHost";
import {
  FreeMediaUploadError,
  type FreeMediaUploadProvider,
  type UguuUploadResponse,
  type TflinkUploadResponse,
} from "../../packages/provider/free-media-upload/src/types";
...
interface StubProvider {
  provider: FreeMediaUploadProvider;
  litterbox: Mock;
  uguu: Mock;
  tflink: Mock;
}
...
function makeStubProvider(overrides: StubOverrides): StubProvider {
  ...
  // FreeMediaUploadProvider's structural .schema field on each upload method.
  const provider = {
  ...
  } as unknown as FreeMediaUploadProvider;
...
describe("uploadToAnyHost", () => {
  ...
  it("throws FreeMediaUploadError aggregating per-host failures when all hosts fail", async () => {
    const { provider } = makeStubProvider({
    ...
    await expect(promise).rejects.toBeInstanceOf(FreeMediaUploadError);
  ...
  it("rejects an empty hosts list", async () => {
    ...
    ).rejects.toMatchObject({ name: "FreeMediaUploadError", status: 400 });
  });
